...
// Input the imu pakcet payload size based on your pcap imu udp packet size. e.g. 48 for standard imu packet.
...
void read_packets(const std::string& pcap_filename,
    ...
    uint32_t num_chans = static_cast<uint32_t>(std::stoul(num_channels)); // Number of channels, e.g. for an OSX-64, num_channels = 64 

    uint32_t azimuths_per_packet = 16;
    uint32_t num_header_footer_words = 5;
    uint32_t bytes_per_word = 4;
    uint32_t words_per_channel = 3;
    uint32_t lidar_payload_size = azimuths_per_packet * bytes_per_word * (num_header_footer_words + (num_chans * words_per_channel));
